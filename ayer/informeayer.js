...
// Guardar el resultado en un archivo JSON en la misma carpeta, actualizando los datos previos
const reportePath = path.join(__dirname, 'reporte_paneles_ayer.json');
let prevPanels = [];
if (fs.existsSync(reportePath)) {
  try {
    prevPanels = JSON.parse(fs.readFileSync(reportePath, 'utf8'));
  } catch (e) {
    prevPanels = [];
  }
}

// Crear un mapa de paneles previos
const prevMap = {};
prevPanels.forEach(p => {
  prevMap[p.panel] = p;
});

// Actualizar o agregar los paneles procesados
reporte.forEach(fila => {
  prevMap[fila.panel] = fila;
});

// Generar el array actualizado
const updatedPanels = Object.values(prevMap);
fs.writeFileSync(reportePath, JSON.stringify(updatedPanels, null, 2), 'utf8');
console.log('Reporte actualizado en reporte_paneles_ayer.json');